...
public sealed class AiStrategyRegistry
{
    ...
    /// <summary>
    /// Возвращает «обученные» стратегии (эволюционировавшие и созданные вручную),
    /// которые можно перенести между установками. Встроенные стратегии не включаются,
    /// так как они заново генерируются из профилей engine/ при синхронизации.
    /// </summary>
    public IReadOnlyList<StrategyGenome> GetTrainedGenomes()
    {
        lock (_gate)
            return _model.Genomes
                .Where(g => g.Origin is StrategyOrigin.Evolved or StrategyOrigin.Manual)
                .ToList();
    }

    public void ExportStrategies(string path)
    {
        var genomes = GetTrainedGenomes();
        var dir = Path.GetDirectoryName(path);
        if (!string.IsNullOrEmpty(dir))
            Directory.CreateDirectory(dir);

        var json = JsonSerializer.Serialize(genomes, JsonOptions);
    ...
    /// <summary>
    /// Импортирует обученные стратегии из JSON-файла. Встроенные стратегии из файла
    /// приводятся к типу Manual. При merge=true существующие стратегии с тем же Id
    /// заменяются; иначе добавляются только новые. Возвращает число добавленных стратегий.
    /// </summary>
    public int ImportStrategies(string path, bool merge = true)
    {
        if (!File.Exists(path))
            return 0;

        List<StrategyGenome>? imported = null;
        try
        {
            var json = File.ReadAllText(path);
            imported = JsonSerializer.Deserialize<List<StrategyGenome>>(json, JsonOptions);
        }
        catch
        {
            return 0;
        ...
        lock (_gate)
        {
            ...
            foreach (var g in imported)
            {
                ...
                var existing = _genomeIdLookup.GetValueOrDefault(g.Id);
