    ...
    private async Task LoopAsync(CancellationToken ct)
    {
        try
        {
            ...
            try
            {
                ...
                // Zapret уже работает при старте оркестратора — пропускаем первичный probing,
                // чтобы не ронять работающий winws.exe конфликтом драйвера WinDivert.
            ...
            try
            {
                if (!_fastStartDone && _aiSettings().FastStartEnabled && !_isAnyEngineRunning())
                {
                    _fastStartDone = true;
            ...
            {
                // Zapret уже работает — не запускаем winws для probing (конфликт драйвера
                // WinDivert роняет работающий zapret и «завешивает» интерфейс). Просто ждём
                // остановки zapret, периодически просыпаясь.
                if (_isAnyEngineRunning())
                {
                    try { await _loopSignal.WaitAsync(CheckInterval, ct).ConfigureAwait(false); }
                ...
                // Повторная проверка прямо перед циклом — zapret мог запуститься за время ожидания.
                if (_isAnyEngineRunning())
                    continue;
