...
///
/// BSDPI is currently distributed as a portable app, so settings intentionally stay
/// next to BSDPI.exe. This service hardens the existing behavior without changing
...
public sealed class SettingsService : ISettingsService
{
    ...
    public void Save(AppSettings settings)
    {
        ...
        {
            Trace.TraceError($"BSDPI settings save failed. Path='{SettingsPath}'. Error='{ex}'");
    ...
    private static bool TryLoad(string path, out AppSettings settings)
    {
        ...
        {
            Trace.TraceError($"BSDPI settings load failed. Path='{path}'. Error='{ex}'");
            return false;
        }
    }
    ...
    private void TryRestoreBackupAsPrimary()
    {
        try
        {
            ...
            Trace.TraceWarning($"BSDPI settings restored from backup. Backup='{BackupPath}', Target='{SettingsPath}'.");
        }
        catch (Exception ex)
        {
            Trace.TraceError($"BSDPI settings backup restore failed. Backup='{BackupPath}', Target='{SettingsPath}'. Error='{ex}'");
        }
    }
