...
public sealed class OrchestratorService : IDisposable
{
    ...
    private readonly Func<bool>? _isZapretRunning;
    ...
    public OrchestratorService(
        ...
        Func<bool>? isZapretRunning = null)
    {
        ...
        _isZapretRunning = isZapretRunning;
    ...
    private async Task ScanAndRankAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Zapret уже работает — запуск winws для probing вызовет конфликт драйвера WinDivert
            // и обрушит работающий zapret (интерфейс «зависает», winws падает). Пропускаем реальное
            // сканирование и просто используем ранее сохранённый рейтинг.
            if (_isZapretRunning is not null && _isZapretRunning())
            {
                Notify("Zapret уже запущен — сканирование профилей пропущено (чтобы не ронять работающий winws.exe).");
                return;
            }
    ...
    private async Task StartBestProfileAsync(CancellationToken ct)
    {
        ...
        // Если zapret уже работает — не перезапускаем winws (это роняет работающий zapret).
        // Выбор оркестратора применится при следующем ручном запуске пользователем.
        if (_isZapretRunning is not null && _isZapretRunning())
        {
            Notify($"Zapret уже запущен — профиль «{best.profile.DisplayName}» выбран, применится при следующем запуске.");
            return;
        }
    ...
    {
        // Zapret работает — не запускаем winws для проверки (конфликт драйвера).
        if (_isZapretRunning is not null && _isZapretRunning())
        {
            Notify("Zapret уже запущен — периодическая проверка профиля пропущена.");
            return;
        }
