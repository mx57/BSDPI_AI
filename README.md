...
- `BSDPI/App.xaml.cs` — добавлены **глобальные** перехватчики, чтобы больше никакой сбой не ронял процесс молча:
...
- Переименованы решение (`BSDPI.slnx`), проекты (`BSDPI`, `BSDPI.Core`, `BSDPI.AI`, `BSDPI.Updater`, `BSDPI.Core.Tests`), папки, namespace'ы, ViewModels/Views, иконки.
...
- Новая вкладка **ByeDPI** (`BSDPI/Views/ByeDPIView`, `ByeDpiEngine`): SOCKS5, порты, параметры запуска `ciadpi.exe`.
