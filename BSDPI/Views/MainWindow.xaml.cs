...
public partial class MainWindow : Window
{
    ...
    {
        _trayIcon.ShowBalloon("BSDPI", $"Профиль переключён: {profileName}");
        _trayIcon.UpdateTooltip($"BSDPI — {profileName}");
    ...
    protected override void OnStateChanged(EventArgs e)
    {
        ...
        if (WindowState == WindowState.Minimized)
        {
            ShowInTaskbar = false;
            Hide();
            _trayIcon.ShowBalloon("BSDPI", "Приложение свёрнуто в трей");
        }
    }
    ...
    protected override void OnClosing(CancelEventArgs e)
    {
        ...
        if (!_isClosingConfirmed)
        {
            ...
            {
                _isClosingConfirmed = true;
                _logger?.LogInformation("User confirmed BSDPI shutdown from main window.");
