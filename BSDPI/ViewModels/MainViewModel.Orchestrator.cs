...
public partial class MainViewModel
{
    ...
    private async Task SwitchProfileAsync(ProfileItem? profile)
    {
        ...
        // Если zapret уже работает — НЕ убиваем и не перезапускаем winws (это роняло
        // работающий zapret и вызывало «зависание» интерфейса). Просто запоминаем выбор
        // оркестратора; он применится при следующем запуске пользователем.
        if (IsRunning)
        {
            if (profile is not null)
            {
