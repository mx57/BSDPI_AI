...
public partial class MainViewModel
{
    ...
    [RelayCommand]
    public async Task ToggleStartStop()
    {
        if (IsRunning)
            await Stop();
        else
    ...
    private async Task Start()
    {
        ...
        {
            SyncCustomHostlist();
            // Полный сброс драйвера WinDivert перед стартом: убиваем осиротевшие winws
            // и выгружаем/загружаем драйвер. Это чистит «призрачные» фильтры, оставшиеся
            // после падений предыдущих запусков, из-за которых новый winws.exe падал
            // через пару секунд (не мог забиндить драйвер). Безопасно, т.к. IsRunning=false.
            await Task.Run(ProfileBatLauncher.ResetWinDivertDriver).ConfigureAwait(false);
            await ProfileBatLauncher.PrepareRuntimeAsync(EngineDir).ConfigureAwait(false);
    ...
    private async Task StartWinwsDirect(WinwsLaunchPlan plan)
    {
        ...
        _ = Task.Run(async () =>
        {
            for (var s = 1; s <= 10; s++)
            {
                ...
                bool exited;
                int code = -1;
                try { exited = winws.HasExited; if (exited) code = winws.ExitCode; }
                catch { exited = true; }
                if (exited) break;
            }
        });
        ...
        Application.Current?.Dispatcher.BeginInvoke(() =>
        {
            ...
            Logs.Add($"winws.exe запущен как дочерний процесс BSDPI, PID: {winws.Id}");
    ...
    private async Task StartByeDpiAsync()
    {
        try
        {
            var profile = _engineManager.CloneWithDefaults(BSDPI.Core.Models.DpiEngineType.ByeDpi);
            profile.SocksPort = ByeDpiSocksPort;
            var ok = await _engineManager.StartAsync(BSDPI.Core.Models.DpiEngineType.ByeDpi, profile);
            if (ok)
            {
    ...
    private async Task Stop()
    {
        ...
        // Снимаем «призрачные» фильтры/драйвер WinDivert, оставшиеся после жёсткого завершения winws.
        _ = Task.Run(ProfileBatLauncher.UnloadWinDivertDriver);
