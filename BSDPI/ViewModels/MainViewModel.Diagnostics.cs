...
public partial class MainViewModel
{
    ...
    private void ExportLogs()
    {
        try
        {
            var dialog = new Microsoft.Win32.SaveFileDialog
            {
                ...
                FileName = $"BSDPI_log_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.txt",
                Title = "Экспорт логов"
            };
            if (dialog.ShowDialog() != true) return;
            var sb = new StringBuilder();
            sb.AppendLine($"BSDPI v{AppVersion} — Лог от {DateTime.Now:dd.MM.yyyy HH:mm:ss}");
    ...
    private void ExportDiagnosticBundle()
    {
        try
        {
            var dialog = new Microsoft.Win32.SaveFileDialog
            {
                ...
                FileName = $"BSDPI_bundle_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.zip",
            ...
            {
                writer.WriteLine($"BSDPI v{AppVersion} — Системный лог [{DateTime.Now:dd.MM.yyyy HH:mm:ss}]");
    ...
    private void CopyExtendedDiagnostics()
    {
        try
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine($"BSDPI v{AppVersion} — Диагностика [{DateTime.Now:dd.MM.yyyy HH:mm:ss}]");
