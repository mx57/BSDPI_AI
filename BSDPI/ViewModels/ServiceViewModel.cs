...
public sealed partial class ServiceViewModel : ObservableObject
{
    ...
    public Func<IEnumerable<BSDPI.Core.Models.TargetEntry>>? GetAutoTuneTargets { get; set; }
    ...
    private async Task RunAutoTuneAsync(CancellationToken ct)
    {
        ...
        var checker = new BSDPI.Core.Services.ConnectivityChecker();
        ...
        var targets = GetAutoTuneTargets?.Invoke()?.Take(5).ToList()
                      ?? BSDPI.Core.Services.ConnectivityChecker.BuiltinSites
        ...
        try
        {
            for (int i = 0; i < combos.Length && !foundPerfect; i++)
            {
                ...
                IReadOnlyList<BSDPI.Core.Models.CheckResult> results;
                try
                {
                ...
                {
                    if (ct.IsCancellationRequested) break;
                    results = System.Array.Empty<BSDPI.Core.Models.CheckResult>();
                }
                sw.Stop();
