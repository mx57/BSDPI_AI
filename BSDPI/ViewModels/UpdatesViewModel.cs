...
public sealed partial class UpdatesViewModel : ObservableObject
{
    ...
    private async Task CheckAppUpdate()
    {
        ...
        AppUpdateStatus = "🔍 Проверяем обновление BSDPI...";
        HasAppUpdate = false;
        ...
        {
            AppUpdateStatus = $"✅ Актуальная версия BSDPI ({CurrentAppVersion})";
            LatestAppVersion = CurrentAppVersion;
            AddLog("✅ BSDPI: обновлений нет");
        }
        else
        {
            ...
            AppUpdateStatus = $"⬆️ Доступна BSDPI v{update.Version}";
            AddLog($"⬆️ BSDPI: доступна версия {update.Version}");
        }
    ...
    private async Task InstallAppUpdate()
    {
        ...
        if (!CustomDialog.Show(
            "⬆️ Обновление BSDPI",
            $"Будет установлена BSDPI v{update.Version}.\n\n" +
        ...
        _addAppLog($"Установка BSDPI v{update.Version}...");
        ...
        if (success)
        {
            AddLog($"✅ BSDPI v{update.Version} установлен, перезапуск...");
    ...
    public async Task AutoDownloadEngineAsync(bool skipConfirmation = false)
    {
        ...
        try
        {
            ...
            if (!skipConfirmation && Application.Current != null && !Application.Current.Dispatcher.HasShutdownStarted)
            {
                confirmed = Application.Current.Dispatcher.Invoke(() =>
                    CustomDialog.Show(
                        "⬇️ Скачивание Flowseal",
                        $"Для работы BSDPI необходим движок Flowseal (v{update.Version}).\n\n" +
