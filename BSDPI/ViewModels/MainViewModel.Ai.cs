...
public partial class MainViewModel
{
    ...
    private void ExportStrategies()
    {
        try
        {
            var sfd = new Microsoft.Win32.SaveFileDialog
            {
                Filter = "BSDPI Strategies (*.json)|*.json|All Files (*.*)|*.*",
                FileName = $"bsdpi-strategies-{DateTime.Now:yyyyMMdd-HHmm}.json",
                Title = "Экспорт обученных стратегий"
            };

            if (sfd.ShowDialog() == true)
            {
                _aiRegistry.ExportStrategies(sfd.FileName);
                Logs.Add($"[ИИ] Стратегии экспортированы в {Path.GetFileName(sfd.FileName)}");
            }
        }
        catch (Exception ex)
        {
            Logs.Add($"[ИИ] Ошибка экспорта стратегий: {ex.Message}");
        }
    }
    ...
    private void ImportStrategies()
    {
        try
        {
            ...
            {
                Filter = "BSDPI Strategies (*.json)|*.json|All Files (*.*)|*.*",
                Title = "Импорт обученных стратегий"
            };

            if (ofd.ShowDialog() == true)
            {
                var imported = _aiRegistry.ImportStrategies(ofd.FileName);
                _aiOrchestrator.SyncRegistryFromEngine();
                RefreshAiDashboard();
                RebuildAiStrategyRows();
                Logs.Add($"[ИИ] Импортировано стратегий: {imported}");
            }
        }
        catch (Exception ex)
        {
            Logs.Add($"[ИИ] Ошибка импорта стратегий: {ex.Message}");
        }
    }
