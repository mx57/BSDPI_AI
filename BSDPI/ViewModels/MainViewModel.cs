...
public partial class MainViewModel : ObservableObject
{
    ...
    [RelayCommand]
    private void CancelProcessPicker()
    {
        ProcessPickerOpen = false;
    }
        ...
        ChainStore chainStore)
    {
        ...
        Service.GetAutoTuneTargets = () =>
        {
            ...
            return sites
                .SelectMany(s => BSDPI.Core.Services.ConnectivityChecker.BuiltinSites.TryGetValue(s, out var e) ? e : Enumerable.Empty<BSDPI.Core.Models.TargetEntry>())
                .ToList();
        };
        ...
        if (!Directory.Exists(EngineDir) || Directory.GetFiles(EngineDir, "*.bat").Length == 0)
        {
            ...
            _ = Updates.AutoDownloadEngineAsync(skipConfirmation: true);
        }
    ...
    .ToList(),
            ...
            TgProxy = new BSDPI.Core.Services.TgProxySettings
            {
                Host = TgProxyHost,
    ...
    {
        SyncHostlistFile("list-general-user.txt", CustomTargetDomains, isExclusion: false);
        SyncHostlistFile("list-exclude-user.txt", CustomExcludeDomains, isExclusion: true);
    ...
    private void SyncHostlistFile(string fileName, ObservableCollection<string> sourceList, bool isExclusion)
    {
        try
        {
            ...
            // 1. Берем из переданного списка (вкладка "Домены")
