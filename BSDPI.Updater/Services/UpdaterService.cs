...
namespace BSDPI.Updater.Services;
...
public sealed partial class WarpUpdaterService : IWarpUpdaterService
{
    ...
    public WarpUpdaterService() : this(new BSDPI.Core.Services.DefaultHttpClientFactory()) { }
    ...
    public string GetLocalVersion(string warpDir)
    {
        ...
        if (File.Exists(path))
        {
            try { return File.ReadAllText(path).Trim().TrimStart('v', 'V'); }
            catch { }
        }
        return "unknown";
    }
    ...
    public async Task<(UpdateInfo? update, string? error)> GetLatestReleaseAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            http.DefaultRequestHeaders.Add("User-Agent", "BSDPI-Warp-Updater");
    ...
    public async Task<bool> InstallUpdateAsync(string warpDir, UpdateInfo update, Action<string> onProgress, CancellationToken ct = default)
    {
        ...
        var tempZip = Path.Combine(Path.GetTempPath(), $"warp_update_{uniqueId}.zip");
        var tempExtract = Path.Combine(Path.GetTempPath(), $"warp_update_{uniqueId}_extract");
...
public sealed partial class ByeDpiUpdaterService : IByeDpiUpdaterService
{
    ...
    public ByeDpiUpdaterService() : this(new BSDPI.Core.Services.DefaultHttpClientFactory()) { }
    ...
    public async Task<(UpdateInfo? update, string? error)> GetLatestReleaseAsync(CancellationToken ct = default)
    {
        try
        {
            using var http = _httpClientFactory.CreateClient(HttpClientNames.Updater);
            http.DefaultRequestHeaders.Remove("User-Agent");
            http.DefaultRequestHeaders.Add("User-Agent", "BSDPI-ByeDpi-Updater");
    ...
    {
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        var tempZip = Path.Combine(Path.GetTempPath(), $"byedpi_update_{uniqueId}.zip");
        var tempExtract = Path.Combine(Path.GetTempPath(), $"byedpi_update_{uniqueId}_extract");
...
public sealed partial class UpdaterService : IUpdaterService
{
    ...
    private const string ApiLatestReleaseUrl =
    ...
    public async Task<(UpdateInfo? update, string? error)> GetLatestReleaseAsync(CancellationToken ct = default)
    {
        try
        {
            using var http = _httpClientFactory.CreateClient(HttpClientNames.Updater);
            http.DefaultRequestHeaders.Remove("User-Agent");
            http.DefaultRequestHeaders.Add("User-Agent", "BSDPI-Updater");

            // 1) GitHub API — самый надёжный способ получить последний релиз и прямую ссылку на zip.
            try
            {
                var json = await http.GetStringAsync(ApiLatestReleaseUrl, ct);
                ...
                if (root.TryGetProperty("tag_name", out var tagEl))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(tag))
                    {
                        ...
                        // Если не удалось вытащить asset, строим URL по шаблону.
                        downloadUrl ??= string.Format(ZipUrlTemplate, tag);
            ...
            catch (HttpRequestException) { /* fallback к version.txt */ }
            catch (JsonException) { /* fallback к version.txt */ }
            catch (KeyNotFoundException) { /* fallback к version.txt */ }

            // 2) Fallback: устаревший файл .service/version.txt
    ...
    {
        var uniqueId = Guid.NewGuid().ToString("N")[..8];
        var tempZip     = Path.Combine(Path.GetTempPath(), $"fluxroute_update_{uniqueId}.zip");
        var tempExtract = Path.Combine(Path.GetTempPath(), $"fluxroute_update_{uniqueId}_extract");
