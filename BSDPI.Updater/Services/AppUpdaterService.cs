...
/// <summary>Информация о доступном обновлении самого приложения BSDPI.</summary>
...
public interface IAppUpdaterService
{
    ...
    /// <summary>Проверяет наличие нового релиза BSDPI через редирект GitHub Releases (без API, без лимитов).</summary>
...
public sealed class AppUpdaterService : IAppUpdaterService
{
    ...
    // Имя asset в релизе: BSDPI_AI-v1.6.0-portable.zip
    private const string AssetNameTemplate = "BSDPI_AI-{0}-portable.zip";
    ...
    private const string UserAgent = "BSDPI-AppUpdater";
        ...
        CancellationToken ct = default)
    {
        ...
        var tempZip  = Path.Combine(Path.GetTempPath(), $"BSDPI_{uniqueId}.zip");
        var tempDir  = Path.Combine(Path.GetTempPath(), $"BSDPI_{uniqueId}_extracted");
        var batPath  = Path.Combine(Path.GetTempPath(), $"_BSDPI_updater_{uniqueId}.bat");

        try
        {
            // ── 1. Скачиваем zip ──────────────────────────────────────────
            onProgress($"⬇️ Скачиваем BSDPI v{update.Version}...");
            ...
            // Ищем BSDPI.exe внутри архива (может быть в подпапке)
            var extractedExe = Directory.GetFiles(tempDir, "BSDPI.exe", SearchOption.AllDirectories)
                                         .FirstOrDefault();
            if (extractedExe is null)
                return (false, "BSDPI.exe не найден внутри архива");
            ...
            // Папка внутри архива, где лежит BSDPI.exe — копируем ВСЁ из неё
            ...
            var bat = $"""
                @echo off
                chcp 65001 > nul
                echo [BSDPI Updater] Ожидаем завершения процесса PID {pid}...
                :waitloop
                ...
                echo [BSDPI Updater] Устанавливаем v{update.Version}...
                xcopy /E /Y /I "{safeExtractedDir}\*" "{safeExeDir}\"
                if errorlevel 1 (
                    echo [BSDPI Updater] Ошибка копирования!
                    pause
                    exit /b 1
                )
                echo [BSDPI Updater] Завершаем дочерние процессы...
                taskkill /IM winws.exe /F > nul 2>&1
                taskkill /IM WinDivert.exe /F > nul 2>&1
                net stop WinDivert > nul 2>&1
                echo [BSDPI Updater] Очищаем временные файлы...
                del /F /Q "{safeTempZip}" > nul 2>&1
                rd /S /Q "{safeTempDir}" > nul 2>&1
                echo [BSDPI Updater] Запускаем BSDPI v{update.Version}...
